...
tool_root = os.path.dirname(inspect.getfile(inspect.currentframe()).replace('\\','/').replace('python3.11libs/pythonrc.py',''))
print(tool_root)
python_root = '%s/python' % tool_root
python_root2 = '%s/python311' % tool_root
if python_root not in sys.path:
    sys.path.append(python_root)
if python_root2 not in sys.path:
    sys.path.append(python_root2)
